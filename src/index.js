...
function str2buf (str, enc) {
  ...
  if (!enc && this.util.isHexString(str)) enc = 'hex'
...
class secKeys {
   ...
   * Verify object whether it is a function
   */
  isFunction (f) {
    return typeof f === 'function'
  }
  /**
   * @param  {} buffer
   * @param  {} {returncreateKeccakHash('keccak256'
   * @param  {} .update(buffer
   * @param  {} .digest(
   */
  keccak256 (buffer) {
...
*/
  ...
  * @param {string} address SEC address.
  ...
  * (Note: Node.js only!)
  * @param {string} address SEC address to import.
  * @param {string=} datadir SEC data directory (default: ~/.SEC).
  * @param {function=} cb Callback function (optional).
